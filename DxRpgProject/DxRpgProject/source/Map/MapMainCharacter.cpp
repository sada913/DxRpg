...
bool MapMainCharacter::move(int *isPassable)
{
    ...
    {
        // BまたはFが押されている
        if (gc->getKey(KEY_INPUT_B) >= GameController::Pressed
            || gc->getKey(KEY_INPUT_F) >= GameController::Pressed)
        {
            if (gc->getKey(KEY_INPUT_B) >= GameController::Pressed)
            {
                walkSpeed_ = 2;
        ...
        // Eが押されていないとき
        if (gc->getKey(KEY_INPUT_E) == GameController::NotPressed)
        {
            int cy = y_ / BlockLen;
            int cx = x_ / BlockLen;
            // バグ回避のため、
        ...
        // キーが押されてない時 (キー入力受付条件)
        if (gc->getKey(KEY_INPUT_LEFT) == GameController::NotPressed
            && gc->getKey(KEY_INPUT_UP) == GameController::NotPressed
            && gc->getKey(KEY_INPUT_RIGHT) == GameController::NotPressed
            && gc->getKey(KEY_INPUT_DOWN) == GameController::NotPressed)
        {
            stop();                                // 歩くカウンタを戻す
        }
        else                                    // キーが何か押されている時
        {
            ...
            {
                // 入力キーの判定
                if (gc->getKey(InputKeyDirection[i]) >= GameController::Pressed)
                {
